...
async function fetchCredits(movieId) {
    try {
        ...
        return { cast, director: director ? director.name : 'Director not available' };
    } catch (error) {
...
Summarize the movie: ${movie.title} - ${movie.overview}. Limit the response to 1500 characters. Also, provide 5 keywords about the movie, separated by commas. Finally, give a single-sentence abstract of what the movie is about. Return only the JSON object.`;
    ...
    try {
        ...
        console.error('Error fetching movie summary, keywords, and abstract:', error);
        return { summary: 'Summary not available', keywords: [], abstract: 'Abstract not available' };
    }
}
...
async function insertIntoUpstash(movie, summaryData, credits, genres, imdbLink) {
    try {
        const data = {
            id: movie.id,
            title: movie.title,
...
(async function () {
    ...
    for (let year = startYear; year <= endYear; year++) {
        ...
        while (currentPage <= totalPages) {
            const { movies, total_pages } = await fetchMoviesByYear(year, currentPage);
            totalPages = total_pages;

            if (movies.length > 0) {
                console.log(`Processing year ${year}, page ${currentPage} of ${totalPages}`);
                for (const movie of movies) {
                    const exists = await movieExists(movie.id);
                    if (exists) {
                        console.log(`Movie ${movie.title} already exists in Upstash. Skipping...`);
                        continue;
                    }
                    ...
                    await insertIntoUpstash(movie, summaryData, credits, genres, imdbLink);
                    ...
                    console.log(`Poster Path: https://image.tmdb.org/t/p/w500${movie.poster_path}`);
                    console.log(`Overview: ${movie.overview}`);
                    console.log(`Genres: ${movie.genre_ids.map((id) => genres[id]).join(', ')}`);
