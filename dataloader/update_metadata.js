...
async function updateMovieMetadata(movie, adult, popularity) {
    try {
        ...
        console.log(`Updated movie ${movie.metadata.name} with adult field in Upstash`);
    } catch (error) {
        console.log(movie)
        console.error(`Error updating movie ${movie.metadata.name} in Upstash:`, error);
    }
}
...
(async function () {
    ...
    do {
        ...
        for (const movie of vectors) {
            ...
            console.log(cursor);
            if (movieDetails) {
                await updateMovieMetadata(movie, movieDetails.adult, movieDetails.popularity);
            }
        }
